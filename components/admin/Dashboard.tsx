...
export default function Dashboard() {
  ...
  const statsCards = [
    ...
    },
    {
      ...
      badge: data.stats.pendingAppointments > 0 ? `${data.stats.pendingAppointments} pending` : undefined,
    },
    {
      title: 'Messages',
      ...
      href: '/admin/contacts',
      badge: data.stats.newMessages > 0 ? `${data.stats.newMessages} new` : undefined,
    },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="grid grid-rows-2 gap-6">
          ...
          <Card className="border-0 shadow-soft">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-semibold">Packages by Location</CardTitle>
            </CardHeader>
              ...
              <div className="h-[120px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.packagesByLocation} layout="vertical">
                    ...
                    />
                    <Bar dataKey="value" fill="#f97316" radius={[0, 4, 4, 0]} name="Packages" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <Card className="border-0 shadow-soft">
          ...
          <CardContent className="space-y-4">
            {data.recentPackages.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">No packages yet</p>
            ) : (
                ...
                >
                  ...
                  <div className="text-right ml-4">
                    ...
                    <p className="text-xs text-muted-foreground flex items-center gap-1 justify-end">
                      <span className="text-yellow-500">★</span> {pkg.rating}
                    </p>
                  </div>
                </div>
              ))
          ...
          <CardContent className="space-y-4">
            {data.recentAppointments.length === 0 ? (
              ...
              data.recentAppointments.map((apt) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-foreground truncate">{apt.fullName || apt.name}</p>
                    <p className="text-sm text-muted-foreground flex items-center gap-1">
                      <Clock className="w-3 h-3" />
                      {apt.service}
                    </p>
                  </div>
                  <Badge className={`ml-4 shrink-0 border-0 ${statusColors[apt.status] || ''}`}>
                    {apt.status}
                  </Badge>
                </div>
              ))
      ...
      <Card className="border-0 shadow-soft">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              { label: 'Add Package', href: '/admin/packages', icon: Package, color: 'bg-orange-500' },
              { label: 'Add Service', href: '/admin/services', icon: Wrench, color: 'bg-blue-500' },
              { label: 'New Blog', href: '/admin/blog', icon: FileText, color: 'bg-purple-500' },
              { label: 'Add Country', href: '/admin/visa', icon: Globe, color: 'bg-green-500' },
              { label: 'Bookings', href: '/admin/appointments', icon: Calendar, color: 'bg-cyan-500' },
              { label: 'Messages', href: '/admin/contacts', icon: Mail, color: 'bg-pink-500' },
            ].map((action) => (
