...
import { Trash2, Edit2, Plus, Globe, Clock, Loader, Search, X } from 'lucide-react';
...
export default function VisaAdminPage() {
  ...
  const handleEdit = (country: VisaCountry) => {
    ...
    setFormData({
      ...
      requirements: country.requirements.length > 0 ? country.requirements : [''],
  ...
  const updateArrayItem = (field: 'requirements' | 'documents' | 'tips', index: number, value: string) => {
    const updated = [...formData[field]];
    updated[index] = value;
    setFormData({ ...formData, [field]: updated });
  };

  const removeArrayItem = (field: 'requirements' | 'documents' | 'tips', index: number) => {
    const updated = formData[field].filter((_, i) => i !== index);
    setFormData({ ...formData, [field]: updated.length > 0 ? updated : [''] });
  };

  const addFee = () => {
    setFormData({ ...formData, fees: [...formData.fees, { type: '', amount: '' }] });
  };

  const updateFee = (index: number, field: 'type' | 'amount', value: string) => {
  ...
  const removeFee = (index: number) => {
    ...
    setFormData({ ...formData, fees: updated.length > 0 ? updated : [{ type: '', amount: '' }] });
  };
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage visa information for countries</p>
        </div>
        <Button
          onClick={() => {
            resetForm();
      ...
      {loading ? (
        ...
        <Card className="border-0 shadow-soft">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              {searchQuery ? 'No countries match your search' : 'No visa countries yet'}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCountries.map((country) => (
            <Card key={country._id} className="border-0 shadow-soft hover:shadow-md transition-shadow">
              <CardContent className="p-5">
                ...
                <div className="mt-4 flex items-center gap-2">
                  ...
                  <Badge variant="outline">{country.requirements.length} requirements</Badge>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent className="w-full sm:max-w-xl">
          <SheetHeader>
            <SheetTitle>{editingId ? 'Edit Country' : 'Add New Country'}</SheetTitle>
            <SheetDescription>
              {editingId ? 'Update visa country details' : 'Add a new visa country'}
            </SheetDescription>
          </SheetHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 py-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="ghost" size="sm" onClick={() => addArrayItem('documents')}>
                    <Plus className="w-4 h-4" />
                  </Button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-sm text-muted-foreground">Visa Fees</h4>
                ...
                {formData.fees.map((fee, index) => (
                  <div key={index} className="flex gap-2">
                    ...
                    <Input
                      ...
                      className="w-32"
                    />
                    <Button type="button" variant="ghost" size="icon" onClick={() => removeFee(index)}>
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="ghost" size="sm" onClick={() => addArrayItem('tips')}>
                    <Plus className="w-4 h-4" />
                  </Button>
                </div>
