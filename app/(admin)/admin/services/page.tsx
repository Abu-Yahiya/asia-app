...
import { Trash2, Edit2, Plus, Wrench, Clock, Loader, Search, X } from 'lucide-react';
...
export default function ServicesAdminPage() {
  ...
  const handleEdit = (service: Service) => {
    ...
    setFormData({
      ...
      process: service.process.length > 0 ? service.process : [{ step: 1, title: '', description: '' }],
    });
    setDrawerOpen(true);
  };
  ...
  const addProcessStep = () => {
    ...
    setFormData({
      ...formData,
      process: [...formData.process, { step: nextStep, title: '', description: '' }],
    });
  };
  ...
  const removeProcessStep = (index: number) => {
    const updated = formData.process.filter((_, i) => i !== index).map((item, i) => ({ ...item, step: i + 1 }));
    setFormData({ ...formData, process: updated.length > 0 ? updated : [{ step: 1, title: '', description: '' }] });
  };
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage your travel services</p>
        </div>
        <Button
          onClick={() => {
            resetForm();
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {filteredServices.map((service) => (
            <Card key={service._id} className="border-0 shadow-soft hover:shadow-md transition-shadow">
        ...
        <SheetContent className="w-full sm:max-w-xl">
          ...
          <ScrollArea className="flex-1 px-1 -mx-1">
            <form onSubmit={handleSubmit} className="space-y-6 py-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-sm text-muted-foreground">Process Steps</h4>
